...
def _migrate_pg():
    ...
    with engine.connect() as conn:
        ...
        try:
            ...
            pass
        try:
            conn.execute(text("ALTER TABLE warranties ADD COLUMN IF NOT EXISTS dispatch_id INTEGER REFERENCES dispatches(id)"))
            conn.commit()
        except Exception:
            pass
        try:
            ...
            pass
        try:
            conn.execute(text("ALTER TABLE inventory_transactions ADD COLUMN IF NOT EXISTS unit_cost VARCHAR(50)"))
            conn.commit()
        except Exception:
            pass
        try:
            conn.execute(text("ALTER TABLE dispatches ADD COLUMN IF NOT EXISTS assigned_to_id INTEGER REFERENCES users(id)"))
            conn.commit()
        except Exception:
            pass
        try:
...
from routers.system import require_module, require_module_write
...
app.include_router(tickets.router,       dependencies=[Depends(require_module("tickets")), Depends(require_module_write("tickets"))])
app.include_router(calendar.router,      dependencies=[Depends(require_module("agenda")), Depends(require_module_write("agenda"))])
...
app.include_router(knowledge_base.router,dependencies=[Depends(require_module("knowledge_base")), Depends(require_module_write("knowledge_base"))])
app.include_router(contacts.router,      dependencies=[Depends(require_module("contactos")), Depends(require_module_write("contacts"))])
app.include_router(companies.router,     dependencies=[Depends(require_module("contactos")), Depends(require_module_write("contacts"))])
app.include_router(opportunities.router, dependencies=[Depends(require_module("oportunidades"))])
app.include_router(quotes.router,        dependencies=[Depends(require_module("cotizaciones")), Depends(require_module_write("quotes"))])
app.include_router(orders.router,        dependencies=[Depends(require_module("pedidos")), Depends(require_module_write("pedidos"))])
app.include_router(invoices.router,      dependencies=[Depends(require_module("facturas")), Depends(require_module_write("facturas"))])
...
app.include_router(suppliers.router,     dependencies=[Depends(require_module("proveedores")), Depends(require_module_write("suppliers"))])
app.include_router(warranties.router,    dependencies=[Depends(require_module("garantias")), Depends(require_module_write("warranties"))])
app.include_router(licenses.router,      dependencies=[Depends(require_module("licencias"))])
app.include_router(contracts.router,     dependencies=[Depends(require_module("contratos")), Depends(require_module_write("contratos"))])
app.include_router(printers.router,      dependencies=[Depends(require_module("impresoras")), Depends(require_module_write("impresoras"))])
app.include_router(supplies.router,      dependencies=[Depends(require_module("suministros")), Depends(require_module_write("suministros"))])
app.include_router(letters.router,       dependencies=[Depends(require_module("cartas")), Depends(require_module_write("cartas"))])
